...
<?php
	...
	$rid = 4; // Retailer role
	$did = 8; // Distributors role
  $aid = 3; // Admin
	$province = $order->delivery_zone; // BC is 67
  $country = $order->delivery_country; // Can 124 US 840
    if ($user->field_hst["und"][0]["value"]){
      $pst = $user->field_hst["und"][0]["value"];
  } else {$pst = NULL;}
  

	$user_retail = user_has_role($rid,$user);
	$user_admin = user_has_role($aid,$user);
	$user_distributors = user_has_role($did,$user);
  
  if ($order->payment_details["cc_type"]){
      $cc_type = $order->payment_details["cc_type"];
  } else {$cc_type = NULL;}
  
	if ($order->payment_method){
     $payment_method  = $order->payment_method;
  } else {$payment_method = NULL;}

	if ($order->data["po_number"]){
       $po_number = $order->data["po_number"];
  } else {$po_number = NULL;} 
 
  $status = $order->order_status;
  $invoiced = gmdate('M d Y', $order->modified);
	$terms = NULL;
  $xport_num = NULL;
  $pst_message = NULL;
  
	if ($order_payment_method == "Due on Receipt"){
		$terms = "Net 21 days";
	}

	if ($user_retail || $user_admin || $user_distributors){
		if ($province == 67){
			if($pst){
          ...
          <td>
            <table width="100%" style="font-family: verdana, arial, helvetica; font-size: small;">
              <tr>
                <td>
                  <?php #print $site_logo; ?>
				  <img typeof="foaf:Image" src="http://www.tgipt.com/sites/all/themes/tgipt/logo.png" alt="The Granville Island Pet Treatery">
                </td>
                <td width="98%">
                  <div style="padding-left: 1em;">
                  <span style="font-size: large;"><?php print $store_name; ?><?php if ($country == 840) { print " (US) Corp";}?></span><br />
                  <?php print $site_slogan; ?>
                  </div>
                </td>
                <td nowrap="nowrap">
                  <?php print $store_address; ?><br />
                    <?php if ($country == 840) { print "1 888 886-7387";} else {print $store_phone;}  ?><br /><br />
                    
                    <?php if ($country != 840) {?>GST: 820 185 403 RT0001<?php } ?>
                    <?php  if ($xport_num) { echo $xport_num;} ?>
                </td>
              </tr>
            </table>
          </td>
              ...
              <tr>
                ...
                <td width="98%">
                  <?php 
                    ...
                    if ($cc_type) { print " ($cc_type)";}
                  ?>

                </td>
              </tr>
              <?php if ($payment_method == "po"){ ?>
                <tr>
                  <td nowrap="nowrap">
                    <b><?php print t('Purchase Order:'); ?></b>
                  </td>
                  <td width="98%">
                    <?php print $po_number; ?>
                  </td>
